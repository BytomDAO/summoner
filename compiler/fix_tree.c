...
eval_compare_expression(Expression *expr) {
    if (expr->u.binary_expression->left->kind == BOOL_EXPRESSION &&
        expr->u.binary_expression->right->kind == BOOL_EXPRESSION) {
...
fix_function_call_expression(Block *current_block, Expression *expr) {
    ...
    for (int i = 0; i < builtin_fun->parameter_count; i++) {
        parameter_list = chain_parameter(parameter_list,
                                         alloc_parameter(alloc_type_specifier(builtin_fun->parameters[i]), ""));
    }
